...
class DenseSenseVectors(SenseVectors):
    ...
    def build(self,
        ...
        vector_dim = wv.vectors.vectors.shape[1]
        senses_num = self.pcz.get_num_senses()
        sv = SenseGram(vector_size=vector_dim, sorted_vocab=0)
        ...
        for word in self.pcz.data:
            for sense_id in self.pcz.data[word]:
                ...
                try:
                    ...
                    sense_vector = np.zeros(wv.vectors.vectors[0].shape, dtype=np.float32) # or the word vector?
                    ...
                    for i, cluster_word in enumerate(self.pcz.data[word][sense_id]["cluster"]):
                        ...
                        # define the word
                        if cluster_word in wv.vectors.key_to_index:
                            cw = cluster_word
                        elif cluster_word.split("#")[0] in wv.vectors.key_to_index:
                            ...
                            for cw in compounds:
                                if cw in wv.vectors.key_to_index and len(cw) > 3:
