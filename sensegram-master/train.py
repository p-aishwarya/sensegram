...
def main():
    ...
    if not exists(neighbours_fpath):
        compute_graph_of_related_words(vectors_fpath, neighbours_fpath, neighbors=100)
    else:
        print("Using existing neighbors:", neighbours_fpath)
        
    if not exists(clusters_fpath):
        word_sense_induction(neighbours_fpath, clusters_fpath, 5, args.threads)
    else:
    ...
    #---------------------------- This part of code is commented, as it is still not working ----------------------
    # building_sense_embeddings(clusters_fpath, vectors_fpath)
    # print("building_sense_embeddings completed!!!!!")
