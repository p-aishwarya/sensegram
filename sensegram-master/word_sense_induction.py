...
def get_ego_network(ego):
    ...
    G = CRSGraph('model/foursquare_sentences.txt.N200.graph')
    ...
    print("Previous length of ego network",ego,len(ego_network),'\n')
    ...
    print("After length of ego network",ego,len(ego_network),'\n')

    chinese_whispers(ego_network, weighting="top", iterations=20)
    if verbose: print("{}\t{:f} sec.".format(ego, time()-tic))
    
    colors = [1. / ego_network.nodes[node]['label'] for node in ego_network.nodes()]
    nx.draw_networkx(ego_network, cmap=plt.get_cmap('jet'), node_color=colors, font_color='White')
