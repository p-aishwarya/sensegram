...
class SenseGram(word2vec.Word2Vec):
    ...
    def create_zero_vectors(self, senses_num, vector_dim):
        ...
        self.wv.vectors = np.zeros((senses_num, vector_dim), dtype=np.float32)
    ...
    def load_word2vec_format(cls, model_fpath, fvocab=None, binary=False, norm_only=True, encoding='utf8', unicode_errors='strict'):
        ...
        result = cls(vector_size=wv_obj.vectors.shape[1])
        result.wv.vectors = wv_obj.vectors
    ...
    def add_sense(self, word, sense_id, vector, prob):
        ...
        if hasattr(self.wv, 'vectors'):

            word_id = len(self.wv.key_to_index)

            sense = word.replace(" ","_") + SEP_SENSE + str(sense_id) # w2v format accepts no whitespaces
            self.wv.key_to_index[sense] = word2vec.Vocab(index=word_id, count=DEFAULT_COUNT)
            self.wv.vectors[word_id] = vector
            self.wv.index2word.append(sense)
            assert sense == self.wv.index2word[self.wv.key_to_index[sense].index]
        else: 
